...
public class Calculos {
	...
	public Double correlacaoDePearson(List<Double> a, List<Double> b, Configuracoes config) {
		...
		for(int i = 0; i< a.size();i++) {// soma da mutiplicação de cada par de elementos
			Double nota1 = a.get(i);
			Double nota2 = b.get(i);
			valor1 += nota1*nota2;
		}
		
		for (Double valor: a) //soma de todos os membros da lista a
			valor2 += valor;
		
		for (Double valor: b) //soma de todos os menbros da lista b
			valor3 += valor;
		
		for(int i = 0; i< a.size();i++) {
			valor4 += Math.pow(a.get(i),2); // soma dos membros da lista a ao quadrado
			valor5 += Math.pow(b.get(i),2); // soma dos membros da lista b ao quadrado
		}
